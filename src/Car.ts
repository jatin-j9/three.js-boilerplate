...
export default class Car {
  ...
  async init(scene: Scene, world: World, position: [number, number, number]) {
    await new GLTFLoader().loadAsync('models/sedanSports.glb').then((gltf) => {
      ...
      const wheelFRBody = world.createRigidBody(
        RigidBodyDesc.dynamic()
          .setTranslation(position[0] + 0.85, position[1], position[2] - 0.63)
          .setCanSleep(false),
      );
      ...
      carMesh.traverse((o) => {
        if (o.type === 'Mesh') {
          const positionAttribute = (o as Mesh).geometry.getAttribute(
            'position',
          );
      ...
      const carShape = (
        ColliderDesc.convexMesh(new Float32Array(positions)) as ColliderDesc
      )
        .setMass(1)
        .setRestitution(0.5)
        .setFriction(3)
      ...
      (this.wheelFLAxel as PrismaticImpulseJoint).configureMotorModel(
      ...
      (this.wheelFRAxel as PrismaticImpulseJoint).configureMotorModel(
        MotorModel.ForceBased,
      );
  ...
  update(delta: number) {
    ...
    (this.wheelBLMotor as PrismaticImpulseJoint).configureMotorVelocity(
      targetVelocity,
      2.0,
    );
    (this.wheelBRMotor as PrismaticImpulseJoint).configureMotorVelocity(
      targetVelocity,
      2.0,
    );
    ...
    (this.wheelFLAxel as PrismaticImpulseJoint).configureMotorPosition(
      targetSteer,
      100,
      10,
    );
    (this.wheelFRAxel as PrismaticImpulseJoint).configureMotorPosition(
      targetSteer,
      100,
      10,
    );
